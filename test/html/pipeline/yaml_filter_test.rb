...
class HTML::Pipeline::YamlFilterTest < Test::Unit::TestCase
  ...
  def test_file_with_no_yaml
    res = markdown_filter("no_yaml.md")

    assert_equal res, load_fixture("after", "no_yaml.text")
  end

  def test_yaml_with_error
    res = markdown_filter("parse_error.md")

    assert_equal res, load_fixture("after", "parse_error.text")
  end
end
