...
class HTMLPipeline
  class EmojiFilterTest < Minitest::Test
    ...
    def test_not_emojify_in_custom_single_tag_foo
      body = "<foo>:shipit:</foo>"
      filter = @emoji_filter.new(body, context: { asset_root: "https://foo.com", ignored_ancestor_tags: ["foo"] })
      doc = filter.call
      assert_equal(body, doc.to_html)
    end
    ...
    def test_not_emojify_in_custom_multiple_tags_foo_and_bar
      body = "<bar>:shipit:</bar>"
      filter = @emoji_filter.new(body, context: { asset_root: "https://foo.com", ignored_ancestor_tags: ["foo", "bar"] })
      doc = filter.call
      assert_equal(body, doc.to_html)
    end
